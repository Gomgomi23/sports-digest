...
작성 지침 — 앵커(Anchor Corp) 리포트 스타일:
- **두괄식**: top3 title은 결론(핵심 팩트)을 먼저. body는 수치·근거·함의 순서.
- **수치 우선**: 금액·비율·기업명 등 구체 숫자가 있으면 반드시 포함. 없으면 생략 (임의 생성 금지).
- **연결 시각**: 스포츠 단독이 아니라 자본·미디어·테크·거버넌스와 어떻게 연결되는지 짚는다.
- **인사이더 톤**: "트렌드다" 대신 "어떤 구조가 이것을 만들었나". 표면 정리가 아닌 구조 해부.
...
- 각 items 항목: "**핵심키워드:** 설명 (수치 포함 시 우선)" 형식으로 한 줄
- 이모지는 섹션 구분자 역할에만, UI 장식용 남발 금지"""
